...
- ELEVATED DRAW PROBABILITY: Knockout 90-minute draw rates are SYSTEMATICALLY and SUBSTANTIALLY higher than group stage, beyond what ranking gap implies. The reasons are structural and have been confirmed by 2026 WC data so far:
  (a) UNDERDOG STRATEGY: Weaker teams know that a 90-min draw gives them ~50% to advance via penalties, versus ~10-15% if they play open and concede early. They deliberately park the bus, absorb pressure, and preserve energy for ET. The rational play for a weaker team is to make it boring.
  (b) FAVORITE PRESSURE: Stronger teams face higher psychological and tactical pressure than in groups. The crowd/media narrative ("must not lose to a weaker team") causes over-caution late in games. The favorites often slow down when drawing, protecting the ET pathway too.
  (c) 2026 TOURNAMENT EVIDENCE: Germany vs Netherlands (R16) ended 1-1 despite a clear paper gap. Multiple R16 games have gone to ET. This is not an anomaly — it is the structural norm of KO football.
  (d) FIRST-GOAL ASYMMETRY: If the underdog scores first (even in the 80th min), the stronger team must open up to equalize, increasing scoring — but if the stronger team scores first, the weaker side has no choice but to attack, making 2-0 more likely. The dangerous scenario for an upset is 0-0 at 70' — underdog maximizes ET entry probability.
  INSTRUCTION: For any match where the underdog has a plausible defensive shape and decent GK, draw probability should be meaningfully higher (≥+5-8pp) vs what naked rankings suggest. Explicitly state in game_script whether the underdog's strategy is "play for ET."
